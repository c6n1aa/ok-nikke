...
ok-nikke-maid 是基于 PyPI `ok-script` 库（ok-script-app 模板）构建的《胜利女神：NIKKE》Windows 客户端 Python GUI 自动化应用。仓库地址：https://github.com/c6n1aa/ok-nikke-maid 。本文是面向 AI Coding Agent的项目说明与约束。
...
scoop 工具（优先于 PowerShell 原生 cmdlet，语法 bash 兼容）：
...
- 文档网站：`.\.venv\Scripts\python.exe -m pip install -r requirements-docs.txt`，然后 `.\.venv\Scripts\python.exe -m mkdocs serve` 或 `.\.venv\Scripts\python.exe -m mkdocs build --strict`。CI 要求 `--strict`。文档为中英双语（`docs/` 中文 + `docs/en/` 英文），必须保持结构对齐。
...
- 以下为被 gitignore 的运行时目录/文件（不要提交）：`configs/`（生成的配置 JSON）、`ok_tasks/`、`ok_templates/`（模板匹配素材）、`screenshots/`、`click_screenshots/`、`logs/`、`cache/`、`site/`、`dev_tools/`（临时脚本与生成的开发产物）、`working/`/`working_images/`（中间帧）、`data/`/`data2/`/`data_export/`/`data_import/`（运行数据）。**特别地，整个 `ok/` 目录与 `ok`、`models/`、`paddle_model/`、`fonts/`、`tesseract/` 也被 gitignore —— ok-script 框架本体不入仓，不要尝试 `import ok.*` 后改其源码**；要扩展/修正 ok 行为只能在 `src/patches/` 走受控补丁。完整列表见 `.gitignore`。
...
- 不要覆写 `click_box`；点击后的等待用 `after_sleep` 参数（详见 `ok-script-codegen` skill 的 Frame Refresh Rules）。
