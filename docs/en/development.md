...
Dependencies are managed by `uv`: `uv sync` builds `.venv` from `uv.lock`; `exclude-dependencies` already drops the `pyside6` meta-package/addons, keeping only `pyside6-essentials`. The shipped lock `requirements.txt` is generated by `uv export --no-dev` for `update.py`'s in-app updates (which still use `pip install --no-deps`) and the CI package interpreter.
