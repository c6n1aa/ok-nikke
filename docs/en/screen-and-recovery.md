...
- **When the same text shows up on several panels, the task side declares the ownership**: "Claim All" is not unique to the login-reward panel (the event mission popup uses the same text in the same lower-screen band); a task owning such a panel must override `_other_claim_all_panel_present()` (`PopupsMixin` defaults to False), otherwise `_close_daily_login_popup` clicks it as if it were the login-reward panel.
- **Pace claiming by "the target popup/screen reappears"** (`dismiss_all_popups(clear_condition=...)`, as `DailyTask._claim_current_tab` does), never by "an overlay must appear": reward overlays are not guaranteed, and multi-stage claiming (the daily-mission tab claims points first and rewards second, in both event and normal missions) has no overlay at all in its first stage — requiring one turns a normal claim into a whole-sub-flow failure.
