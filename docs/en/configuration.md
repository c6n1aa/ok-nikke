...
- The release ships a single portable package; the update source is the runtime setting `configs/update.json` (`channel`: `auto`/`github`/`cnb`), switchable under "About → App update". `auto` follows the system language (Chinese → CNB mirror, otherwise → GitHub). The implementation is `update.py` in the package root; see [Packaging and release](release.md).
