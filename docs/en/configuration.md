...
- The dependency source is `pyproject.toml`: `uv lock` produces `uv.lock`, and `uv export --format requirements-txt --no-hashes --no-dev -o requirements.txt` exports the shipped lock; `[tool.uv] exclude-dependencies` already drops the `pyside6-fluent-widgets -> pyside6` meta-package edge (only `pyside6-essentials` is installed). **When upgrading the framework (`ok-script`), bump `pyproject.toml` and re-run `uv lock` + `uv export`, and commit them with the tag** — in-app updates only reinstall dependencies when the requirements fingerprint changes.
- The release ships a single portable package; the update source and the dependency index are the runtime setting `configs/update.json` (`channel`: `auto`/`github`/`cnb`; `pip_index`: `auto`/`pypi`/`tuna`), switchable under "About → App update". `auto` follows the system language (Chinese → CNB mirror / Tsinghua pip mirror, otherwise → GitHub / official PyPI). The implementation is `update.py` in the package root; see [Packaging and release](release.md).
