...
2. 运行 `ok-nikke.exe`（需要管理员权限，会自动提权，弹 UAC 时点「是」）。
3. 启动游戏本体。
...
- 任务运行期间，若游戏窗口失去焦点或不在前台，应用会自动暂停并在切回游戏窗口后继续，避免点击落空。
- 任务开关与配置项都在主界面选择；程序按日/周记录完成状态，已完成的任务不会被重复执行。

## 应用内更新

「关于 → 应用更新」里可以检查更新、切换更新源（自动 / GitHub / CNB 镜像）与依赖镜像源（自动 / 官方 PyPI / 清华镜像）：

- 更新源出厂为「自动」：中文系统默认走 CNB 国内镜像，其它系统走 GitHub。
- 镜像源出厂为「自动」：中文系统默认走清华镜像，其它走官方 PyPI。
- 更新时会自动拉取代码，必要时重装依赖，完成后应用自动重启；**更新过程会打开一个控制台窗口显示进度**（拉代码 / 装依赖 / 失败原因），日志同时写在包内 `logs/update.log`，失败原因也会在「关于 → 应用更新」里回显。

## 常见问题

- **截图全黑或识别不到画面**：确认游戏同样以管理员权限运行（程序自身会自动提权），并关闭 HDR 或允许 AutoHDR 提示。
- **游戏画面被覆盖层干扰**：关闭其他可能在游戏上添加覆盖显示层的软件（如微星小飞机、游戏加加等）。
