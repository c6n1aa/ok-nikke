...
from src.tasks.event._const import (  # 常量与身份工具集中处（避免 mixin 反向依赖本模块构成环）。
    _MENU_BAND_BOXES, _STORY_MODES, _SWEEP_STAGES, _SWEEP_STAGE_DEFAULT, normalize_roman_numerals,
...
class EventTask(EventListMixin, EventStoryMixin, EventStageMixin, EventChallengeMixin, EventMissionMixin, EventCheckinMixin, EventClaimMixin, EventCommonMixin, NikkeBaseTask):  # 活动任务：自动处理限时活动的通用内容（签到/剧情/挑战/任务/商店）。
