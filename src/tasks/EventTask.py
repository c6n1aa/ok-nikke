...
from src.tasks.event._const import (  # 常量与身份工具集中处（避免 mixin 反向依赖本模块构成环）。
    _EVENT_ICON, _MENU_BAND_BOXES, _MAX_CARDS, _STORY_MODES, _STORY_BATTLE_TIMEOUT, _STORY_MAX_BATTLES,
    _STORY_FIELD_CHANGED_FEATURE, _STORY_FIELD_CHANGED_WAIT, _STORY_MAX_PUSH_ROUNDS, _STAGE_ENTER_TIMEOUT,
    _STAGE_DETAIL_BATTLE_BOX, _BATTLE_AFTER_SLEEP, _SWEEP_STAGES, _SWEEP_STAGE_DEFAULT, _SWEEP_MAX_ROUNDS,
    _SWEEP_QUICK_BOX, _SWEEP_CLOSE_FEATURE, _SD_ARRIVE_TIMEOUT, normalize_roman_numerals, _STORY_MENU_PATTERNS,
    _STORY_SUB_PATTERN, _ENTRY_LOCK_BRIGHT_V, _ENTRY_LOCK_BRIGHT_RATIO, _MENU_PROBE_ENTRIES, _ENTRIES,
    _ENTRY_EXTRA_BOXES, _SUBFLOW_ORDER, _SUBFLOW_METHODS, _SKIPPED_ENTRIES, _ENTRY_CLICK_Y_OFFSET,
    _ENTRY_EXTRA_CLICK_Y_OFFSET, _EVENT_KEY_PREFIX, _EVENT_FLOW_PERIOD, _PER_EVENT_FLOWS, _LEGACY_DONE_KEY,
    _BIG_EVENT_TYPE, event_identity, event_done_key,
)


class EventTask(EventStageMixin, EventChallengeMixin, EventMissionMixin, EventCheckinMixin, EventClaimMixin, EventCommonMixin, NikkeBaseTask):  # 活动任务：自动处理限时活动的通用内容（签到/剧情/挑战/任务/商店）。
