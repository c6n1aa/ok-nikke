import re  # 正则模块，「确定」结果按钮 OCR 部分匹配。
...
# 招募结果「确定」按钮 OCR 匹配模式：OCR 文本常带尾随标点/拆框，用正则部分匹配。
_CONFIRM_PATTERN = re.compile(r"确定")
...
class RecruitTask(NikkeBaseTask):  # 招募任务：每日免费招募/友情点招募/折扣普通招募。
    ...
    def _do_recruit(self):  # 招募整体流程（re-entrant，由 try_step 包裹）：进入招募界面→按配置执行各子招募→返回大厅。
        ...
        if self.config.get("友情点招募"):  # 启用友情点招募才执行。
            self._do_social_point_recruit()  # 友情点招募：翻页找入口→单抽→跳过动画→点确定。
        if self.config.get("折扣普通招募"):  # 启用折扣普通招募才执行。
            self._do_ordinary_recruit()  # 折扣普通招募：翻页找入口→确认→跳过动画→点确定。
    ...
    def _do_social_point_recruit(self):  # 友情点招募：找入口→点击单抽→跳过动画→等待结果确定。
        ...
        self._click_recruit_result()  # 等待结果面板「确定」按钮并点击关闭。

    def _do_ordinary_recruit(self):  # 折扣普通招募：找入口→确认→跳过动画→等待结果确定。
    ...
    def _click_recruit_result(self, time_out=15, interval=0.5):  # 等待招募结果「确定」按钮并点击：0.5秒间隔 OCR，出金/new 遮挡时点击结果区一次再识别。
        ...
        while time.time() < deadline:  # 循环直到超时。
            ...
            matches = self.ocr(box=result_box, match=_CONFIRM_PATTERN)  # 结果面板区域 OCR 部分匹配「确定」。
            if matches:  # 识别到确定按钮。
                self.click_box(matches[0], after_sleep=1)  # 点击确定按钮关闭招募结果面板。
                return  # 结束本子招募。
            self.sleep(interval)  # 间隔 0.5 秒再识别。
            if not retried:  # 首次识别超时：点击结果区一次再识别（出金/new 动画遮挡关键字）。
                self.log_warning("招募结果区域未识别到「确定」，点击结果区后重试。")  # 记录重试原因。
                self.click_box(result_box, after_sleep=interval)  # 点击结果区关闭动画遮罩。
                self.next_frame()  # 刷新一帧后再进入下一轮识别。
                retried = True  # 置位标记，不再重复点击。
        raise WaitFailedException("招募结果「确定」按钮等待超时")  # 超时抛异常由 try_step 恢复。