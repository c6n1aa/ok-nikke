...
# 异常个体队伍序号下拉选项（存储为字符串，与 UI 下拉显示的键一致）。
_ANOMALY_TEAM_OPTIONS = ["1", "2", "3", "4", "5"]

# 切换异常个体 BOSS 的最大次数：BOSS 共 5 个，10 次足以覆盖一轮循环并容忍 OCR 抖动，
# 用尽仍未匹配说明界面状态异常，抛 WaitFailedException 由 try_step 恢复，避免死循环。
_ANOMALY_BOSS_MAX_SWITCH = 10

# 异常个体队伍激活确认的最大补点次数：连续点满仍判态未激活判定为选择失败。
_ANOMALY_TEAM_MAX_ACTIVATE = 5
...
class ArkTask(NikkeBaseTask):  # 方舟任务：执行企业塔/模拟室/拦截战/竞技场等子流程。
    ...
    def __init__(self, *args, **kwargs):  # 初始化任务元数据与配置。
        ...
        self.default_config.update({  # 子任务专属设置，独立持久化到 configs/。
            ...
            "拦截战": False,  # 进行特殊拦截战（与异常拦截战互斥，不可同时开启）。
            "异常拦截战": True,  # 使用快速战斗进行异常个体拦截战扫荡（与拦截战互斥，不可同时开启）。
            "只进行快速战斗": True,  # 异常个体拦截战只进行每周快速战斗。
            "BOSS选择": "克拉肯",  # 异常个体拦截战的目标 BOSS。
            "异常拦截队伍配置": True,  # 按 BOSS 选择对应队伍，关闭则固定使用队伍1。
            "克拉肯": "1",  # 克拉肯使用的队伍序号。
            "镜像容器": "2",  # 镜像容器使用的队伍序号。
            "茵迪维利亚": "3",  # 茵迪维利亚使用的队伍序号。
            "过激派": "4",  # 过激派使用的队伍序号。
            "死神": "5",  # 死神使用的队伍序号。
        ...
        self.config_description.update({  # 每个配置项的帮助文本。
            ...
            "拦截战": "进行特殊拦截战(与异常拦截战互斥，不可同时开启)",
            "异常拦截战": "使用快速战斗进行异常个体拦截战扫荡(与拦截战互斥，不可同时开启)",
            "只进行快速战斗": "开启后<b>只进行每周快速战斗</b>，不可进行快速战斗时跳过拦截战任务流程"
                         "(建议每周手动打一次，后续使用快速战斗进行扫荡)<br/>关闭后<b>当快速战斗不可用时</b>，将进行<b>普通自动战斗</b>",
            "BOSS选择": "选择异常个体",
            "异常拦截队伍配置": "异常个体对应队伍选择，关闭则固定使用队伍1",
            "克拉肯": "异常个体「克拉肯」使用的队伍",
            "镜像容器": "异常个体「镜像容器」使用的队伍",
            "茵迪维利亚": "异常个体「茵迪维利亚」使用的队伍",
            "过激派": "异常个体「过激派」使用的队伍",
            "死神": "异常个体「死神」使用的队伍",
        ...
        self.config_type.update({  # 配置类型与显隐控制：布尔开关联动子配置显隐（参考 ShopTask）。
            ...
            "异常拦截战": {  # 布尔开关，启用时才展开异常个体拦截战配置。
                "sub_configs": {  # 开关联动子配置显隐。
                    True: ["只进行快速战斗", "BOSS选择", "异常拦截队伍配置"],  # 启用时显示战斗方式、BOSS 与队伍配置。
                    False: [],  # 关闭时收起配置。
                },
            },
            "异常拦截队伍配置": {  # 布尔开关，启用时才展开各 BOSS 的队伍序号配置。
                "sub_configs": {  # 开关联动子配置显隐。
                    True: list(_ANOMALY_BOSSES),  # 启用时显示各 BOSS 的队伍序号。
                    False: [],  # 关闭时收起配置（固定使用队伍1）。
                },
            },
            "BOSS选择": {  # 目标异常个体下拉单选。
                "type": "drop_down",  # 下拉选项类型。
                "options": list(_ANOMALY_BOSSES),  # 5 个可选异常个体。
            },
            **{boss: {  # 各 BOSS 的队伍序号下拉单选。
                "type": "drop_down",  # 下拉选项类型。
                "options": list(_ANOMALY_TEAM_OPTIONS),  # 队伍序号 1-5。
            } for boss in _ANOMALY_BOSSES},
    ...
    def _do_interception(self):  # 拦截战子流程：通用拦截战与异常个体拦截战互斥二选一。
    ...
    def _do_interception_flow(self):  # 拦截战整体流程：方舟→拦截战入口→按配置分流到通用/异常个体标签页→返回方舟。
        self._nav_to_ark()  # 确保处于方舟界面（正常已就位；失败恢复回大厅后由此重新进入）。
        self.wait_click_feature("ark_interception", raise_if_not_found=True, after_sleep=2)  # 点击拦截战入口。
        # 入口停留的上次标签页不确定：active/disable 特征成对消歧，等两个标签页判定其一命中。
        if not self.wait_until(  # 轮询判定已进入拦截战相关界面（任一标签页）。
                lambda: self.is_screen("interception_page") or self.is_screen("anomaly_interception_page"),
                time_out=15, settle_time=2):  # 界面加载与过场动画容忍窗口；settle 要求页面特征稳定命中，避免动画早期单帧命中即分流。
            raise WaitFailedException("点击拦截战入口后未进入拦截战界面或异常个体拦截战界面")  # 抛异常由 try_step 恢复。
        if self.config.get("拦截战"):  # 启用通用拦截战。
            self._do_common_interception()  # 执行通用拦截战流程。
        else:  # 启用异常个体拦截战。
            self._do_anomaly_interception()  # 执行异常个体拦截战流程。

    def _do_common_interception(self):  # 通用拦截战：确保通用标签页→进入拦截战→快速/普通战斗循环→返回方舟。
        if not self.is_screen("interception_page"):  # 当前不在通用标签页（落在异常个体标签页）。
            self.assert_screen("anomaly_interception_page")  # 确认处于异常个体标签页。
            self.transition("interception_page", click_feature="common_interception_disable", wait_confirm=10, after_sleep=1)  # 点通用拦截战标签切换界面。
        self.transition("common_interception_page", box="box_common_interception_entry", wait_confirm=10, after_sleep=1)  # 点通用拦截战入口并确认进入。
        while True:  # 战斗循环：快速战斗与普通战斗均不可用时结束。
            quick = self.get_box_by_name("box_common_interception_quick_battle_feature")  # 快速战斗可用性判定区域。
            if quick is not None and self.is_feature_enabled(quick):  # 快速战斗可用。
                self.click_box(quick, after_sleep=2)  # 点击快速战斗并等待结算。
            ...
            self._wait_interception_battle("common_interception_page")  # 战斗等待并确认回到本页。
        self._back_through_screens("interception_page", "ark")  # 逐级返回方舟界面。

    def _do_anomaly_interception(self):  # 异常个体拦截战：匹配 BOSS→（队伍选择）快速/普通战斗循环→返回方舟。
        if not self.is_screen("anomaly_interception_page"):  # 当前不在异常个体标签页（落在通用标签页）。
            self.assert_screen("interception_page")  # 确认处于通用标签页。
            self.transition("anomaly_interception_page", click_feature="anomaly_interception_disable", wait_confirm=10, after_sleep=1)  # 点异常个体拦截战标签切换界面。
        enter = self.get_box_by_name("box_anomaly_interception_battle_enter")  # 战斗入口可用性判定区域。
        ...
        self.transition("anomaly_interception_team_select_page", box="box_anomaly_interception_battle_enter", wait_confirm=10, after_sleep=1)  # 点战斗入口并确认进入队伍选择界面。
        while True:  # 战斗循环：快速战斗与普通战斗均不可用时结束。
            quick = self.get_box_by_name("box_anomaly_interception_quick_battle_feature")  # 快速战斗可用性判定区域。
            if quick is not None and self.is_feature_enabled(quick):  # 快速战斗可用。
            ...
            self._wait_interception_battle("anomaly_interception_team_select_page")  # 战斗等待并确认回到本页。
        self._back_through_screens("anomaly_interception_page", "ark")  # 逐级返回方舟界面。
    ...
    def _match_anomaly_boss(self):  # 切换异常个体直到与配置的 BOSS选择 一致（OCR 部分匹配兼容标点噪声）。
        ...
        for _ in range(_ANOMALY_BOSS_MAX_SWITCH):  # 有限次切换，避免 OCR 抖动导致死循环。
            self.wait_click_feature("anomaly_interception_object_selector", raise_if_not_found=True, after_sleep=1)  # 点击异常个体选择器切换。
            if self._anomaly_boss_matches(pattern):  # 切换后命中目标 BOSS。
                return  # 匹配完成。
        raise WaitFailedException(f"切换异常个体达到上限，未匹配到 BOSS：{boss}")  # 抛异常由 try_step 恢复。

    def _anomaly_boss_matches(self, pattern):  # OCR 判定当前异常个体是否为配置目标。
        object_box = self.get_box_by_name("box_anomaly_interception_object")  # 当前异常个体名标注区域。
        return object_box is not None and bool(self.ocr(box=object_box, match=pattern))  # 区域内 OCR 部分匹配目标名。

    def _select_anomaly_team_if_configured(self):  # 按配置选择队伍：关闭队伍配置时固定使用队伍1（不点击）。
        if not self.config.get("异常拦截队伍配置"):  # 已关闭队伍配置。
            return  # 固定使用队伍1。
        team = self.config.get(self.config.get("BOSS选择"))  # 以 BOSS 名为键读取队伍序号（1-5）。
        team_name = f"box_anomaly_interception_team{team}"  # 组合对应队伍区域特征名。
        team_box = self.get_box_by_name(team_name)  # 获取队伍区域。
        if team_box is None:  # 区域缺失。
            raise WaitFailedException(f"缺少区域特征: {team_name}")  # 抛异常由 try_step 恢复。
        for _ in range(_ANOMALY_TEAM_MAX_ACTIVATE):  # 判态未激活则补点，最多补点 5 次。
            if self.is_feature_enabled(team_box):  # 队伍已激活为当前选择。
                return  # 选择完成。
            self.click_box(team_box, after_sleep=1)  # 点击该队伍并等待其激活。
        raise WaitFailedException(f"队伍{team}激活失败: {team_name}")  # 抛异常由 try_step 恢复。

    def _wait_interception_battle(self, return_screen):  # 拦截战战斗等待：节流等待战斗结束并点击结算按钮返回目标界面。
