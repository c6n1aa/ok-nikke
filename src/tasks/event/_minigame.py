...
from src.tasks.event._const import (
    _MINIGAME_BACK_ATTEMPTS,
    _MINIGAME_CLAIM_MAX_CLICKS,
    _MINIGAME_CLAIM_SETTLE,
    _MINIGAME_DIALOG_TIMEOUT,
    _MINIGAME_ENTER_TIMEOUT,
    _MINIGAME_EXIT_CONFIRM,
    _MINIGAME_EXIT_TIMEOUT,
    _MINIGAME_MAIN_SCREEN,
    _MINIGAME_MISSION_CLAIM_BOX,
    _MINIGAME_MISSION_CLOSE,
    _MINIGAME_MISSION_ENTRY,
    _MINIGAME_MISSION_POPUP_SCREEN,
    _MINIGAME_PAUSE_BOX,
    _MINIGAME_PAUSE_DIALOG_SCREEN,
    _MINIGAME_PLAY_SCREEN,
    _MINIGAME_PLAY_TIMEOUT,
    _MINIGAME_QUICK_FINISH_BOX,
    _MINIGAME_RESULT_BACK_BOX,
    _MINIGAME_RESULT_SCREEN,
    _MINIGAME_RESULT_TIMEOUT,
    _MINIGAME_SCORE_BOX,
    _MINIGAME_SCORE_INTERVAL,
    _MINIGAME_SCORE_PATTERN,
    _MINIGAME_SELECT_SCREEN,
    _MINIGAME_SELECT_START,
    _MINIGAME_START_ENTRY,
    _MINIGAME_START_TIMEOUT,
    _MINIGAME_TAP_INTERVAL,
    _MINIGAME_TAP_POINT,
    _MINIGAME_TARGET_SCORE,
    MINIGAMES,
)
...
class EventMinigameMixin:
    ...
    def _wait_click_feature(self, feature, label, time_out=_MINIGAME_DIALOG_TIMEOUT):  # 等特征出现即点击（模态框内按钮共用）。
        try:  # 特征名可能尚未标注进 coco（旧 assets）。
            ...
            raise WaitFailedException(f"缺少特征: {feature}（{label}）") from None  # 转成可恢复的等待失败，交 try_step 统一处理。
