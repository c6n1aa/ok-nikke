...
2. 运行 `ok-nikke.exe`（需要管理员权限，会自动提权）。
3. 启动游戏本体
...
- 任务运行期间游戏窗口失焦或不在前台会自动暂停，切回游戏窗口后继续。
...
| 方舟 | 企业塔 / 模拟室 / 拦截战 / 竞技场 |
| Raid | 限时挑战（协同作战 / 个人突袭） |
| 活动 | 限时活动通用处理（签到印章 / 剧情推图(BETA)+扫荡 / 挑战 / 活动任务奖励） |

除「日常」外，上表每个任务都可在“任务”界面单独点击运行；也可先在“日常设置”里开启对应任务，再由「日常」把它们编排进一条队列依次执行。
...
3. **显示设置**：
   - 关闭 Windows 自动 HDR。
   - 画质设置：推荐越高越好。
   - 关闭其他可能在游戏上添加覆盖显示层的软件（如微星小飞机、游戏加加等）。
4. **游戏语言**：优先使用简体中文。
