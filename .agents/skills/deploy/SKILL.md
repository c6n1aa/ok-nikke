...
- The release artifact is one portable zip. `python/`, `git/` and the entry exe cannot be delivered by git updates: when a release changes `launcher/` or the package layout, the release notes must tell users to re-download the package. Release notes are generated by CI (`.github/scripts/release_notes.py`) and prefer the user-facing `changelog/<tag>.md` this workflow writes when the user asks for release notes; otherwise CI uses the auto-classified Conventional Commit list. See `docs/release.md`.
...
5. Release notes are **opt-in**: only when the user explicitly asks for them (e.g. "deploy 并生成更新日志", "write the release notes") write `changelog/<tag>.md` before committing, so it ships inside the tag and becomes the GitHub Release changelog. Otherwise skip this step - CI falls back to the auto-classified commit list (see `docs/release.md`).
   - Gather the release scope: `git log --no-merges --format=%s <previous-tag>..HEAD` for committed work and `git diff <previous-tag>` for the uncommitted changes (`git describe --tags --abbrev=0 --match "v*" HEAD` finds the previous tag; run `git fetch --tags origin` first if it looks stale).
   - Rewrite it for players in Chinese: what they can now do, what got better, what was broken and is fixed. Group bullets under `#### 新增` / `#### 优化` / `#### 修复` (that heading level nests under the generated `### 更新日志`; never add a `### 更新日志` heading yourself). Do not copy commit titles or file names, and drop internal-only work (docs, CI, tests, refactors) unless users can notice it.
   ...
   - Call out anything users must do as its own bullet, e.g. re-download the complete portable package when `launcher/` or the package layout changed (in-app git updates cannot deliver the entry exe).
   - Optional sanity check: `.\.venv\Scripts\python.exe .github\scripts\release_notes.py --tag <tag> --out release_notes.md` renders the full release body from the file (before the tag exists it just omits the compare link; `release_notes.md` is git-ignored).
6. Stage only the intended files (including `changelog/<tag>.md` when written), inspect `git diff --cached`, then create the commit:
